...
public class Test {
  ...
  public static void main(final String[] args) {
    final EntityMetadata personMetadata = BeanEntityMetadataFactory.generateMetadata(Person.class);
    final EntityMetadata dogMetadata = SimpleEntityMetadataBuilder.create("Dog")
        .field(create("name", String.class, false)).field(create("color", Color.class, false))
        .build();

    final EntityRepository repository = createRepository(Arrays.asList(personMetadata, dogMetadata),
        person("Hans", "Müller", createDate(7, 11, 1960)),
        person("Rüdiger", "Huber", createDate(3, 5, 1961)), dog("Bello", Color.BLACK),
        dog("Fritz", Color.BROWN));
  ...
  private static void queryAndPrint(final EntityRepository repository, final String name) {
    final EntityRepositoryTemplate template = new EntityRepositoryTemplate(repository);

    System.out.println(name + "\n--------");
    final List<Entity> result = repository.query((FullQuery) () -> name);

    for (final Entity entity : result) {
      System.out.println(EntityPrettyPrinter.print(entity, template.metadata(entity.getName())));
    }
    System.out.println();
  }
  ...
  private static Entity person(final String firstName, final String lastName,
    ...
    return new BeanWrapperEntity<>(new Person(firstName, lastName, birthDate));
  }
  ...
  private static EntityRepository createRepository(final Iterable<EntityMetadata> metadata,
      final Entity... entities) {
    final InMemoryRepository repository = new InMemoryRepository(metadata);
    for (final Entity entity : entities) {
      repository.add(entity);
    }
    return new EnhancerEntityRepository(repository, new PersonEnhancerStrategy());
  }
